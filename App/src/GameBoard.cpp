...
std::bitset<512> GameBoard::bitsToState;
bool GameBoard::bitsToStateSetup;

GameBoard::GameBoard() : m_chunks() {
  if (!GameBoard::bitsToStateSetup) {
    for (int16_t i = 0; i <= 0b111111111; i++) {
      ...
      GameBoard::bitsToState[i] =
          neighbor_count == 3 || (neighbor_count == 2 && alive);
    }
  }
};
...
std::optional<std::reference_wrapper<Chunk>> GameBoard::getChunk(int32_t x,
                                                                 int32_t y) {
...
void Chunk::processNextState() {
  uint64_t up;
  uint64_t current = m_dataBuffer[m_currBuffer][0].to_ulong();
  uint64_t down = m_dataBuffer[m_currBuffer][1].to_ulong();
  // Inside
  for (int i = 1; i < Chunk::Size - 1; i++) {
    ...
    for (int j = 1; j < Chunk::Size - 1; j++) {
      ...
      // Top 3 bits
      around |= (up >> ((Chunk::Size - 8) - i)) & 0b111000000;

      // Middle 3 bits
      around |= (current >> ((Chunk::Size - 5) - i)) & 0b000111000;

      // Bottom 3 bits
      around |= (down >> ((Chunk::Size - 2) - i)) & 0b000000111;

      m_dataBuffer[m_currBuffer ^ 1][i][j] = GameBoard::bitsToState[around];
    }
  }
}

void Chunk::swapToNextState() { m_currBuffer ^= 1; }

bool Chunk::empty() {
  std::bitset<Chunk::Size> val;
